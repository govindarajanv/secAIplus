...
- Current shared cram version: **v1.3** — files `chapter-{1..6}-exam-cram-v1.3.md`. All six share the same version; bump together (minor = content add/fix, major = restructure).
- Chapter 1 "Practice Test Failures" holds 7 exam-ready items (deep learning vs Python analysis; explicit output format; GBDT phishing scoring; federated learning scenario w/ distractor analysis; adversarial stress-test accuracy; RAG vector storage protection; RL state→action→reward cycle).
- Chapter 2 "Practice Test Failures" holds 4 exam-ready items (MIT AI Risk Repository 3 parts incl. 2,244-record database + causal/domain taxonomies; gateway endpoint access controls; third-party pre-trained model deployment risks & vendor evaluation audit; AI threat modeling vs. user access log review w/ distractor analysis).
- `index.html` is a self-contained prep app that EMBEDS copies of the crams (CRAMS_DATA) and flashcards (CARDS_DATA, 123 cards, ids 1–117 original + ch1 failure cards 113–119 + ch2 failure cards 120–123). Brand header displays live version badge (`Cram Notes v1.3`). After any cram edit, sync it: replace matching chapter's `raw`, add/update flashcards, fix UI count strings (meta description, Flashcards (N) button, countTotal, cardCounter, backCounter, navCounter). Validate with `node --check` on the extracted main script + count occurrences of card ids to catch duplicate-array splices. Serialize both data arrays as single-line JSON; verify GRC chapterTitle hits in CARDS_DATA == 21 to detect duplication corruption.
